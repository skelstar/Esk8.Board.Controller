...
void task_1(void *pvParameters)
{
  ...
  while (true)
  {
    if (since_took_semaphore_task_1 > 10000)
    {
      since_took_semaphore_task_1 = 0;

      while (waiting_for_semaphore(xTheSemaphore, 200))
      {
      ...
      while (since_started_task_0_loop < 4000)
      {
        /*
...
void task_2(void *pvParameters)
{
  ...
  while (true)
  {
    if (task_2_try_take_semaphore > 3000)
    {
      task_2_try_take_semaphore = 0;

      while (waiting_for_semaphore(xTheSemaphore, /*ticks*/200))
      {
      ...
      while (since_started_task_2_loop < 2000)
      {
        /*
        if you remove the taskDelay, then other threads will not be switched to
        by the task scheduler until the vTaskDelay(10) below
