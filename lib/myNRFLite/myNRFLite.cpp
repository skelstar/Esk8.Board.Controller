...
uint8_t myNRFLite::init(uint8_t radioId, uint8_t miso_pin, uint8_t mosi_pin, uint8_t sclk_pin, uint8_t cePin, uint8_t csnPin, Bitrates bitrate, uint8_t channel)
{
  _cePin = cePin;
  _csnPin = csnPin;
...
uint8_t myNRFLite::init(uint8_t radioId, uint8_t cePin, uint8_t csnPin, Bitrates bitrate, uint8_t channel)
{
...
uint8_t myNRFLite::initTwoPin(uint8_t radioId, uint8_t momiPin, uint8_t sckPin, Bitrates bitrate, uint8_t channel)
{
    _cePin = sckPin;
    _csnPin = sckPin;
...
void myNRFLite::addAckData(void *data, uint8_t length, uint8_t removeExistingAcks)
{
...
uint8_t myNRFLite::hasData(uint8_t usingInterrupts)
{
...
void myNRFLite::readData(void *data)
{
...
uint8_t myNRFLite::send(uint8_t toRadioId, void *data, uint8_t length, SendType sendType)
{
...
void myNRFLite::startSend(uint8_t toRadioId, void *data, uint8_t length, SendType sendType)
{
    prepForTx(toRadioId, sendType);
...
void myNRFLite::whatHappened(uint8_t &txOk, uint8_t &txFail, uint8_t &rxReady)
{
    uint8_t statusReg = readRegister(STATUS_NRF);
...
uint8_t myNRFLite::getPipeOfFirstRxPacket()
{
...
uint8_t myNRFLite::getRxPacketLength()
{
...
uint8_t myNRFLite::initRadio(uint8_t radioId, Bitrates bitrate, uint8_t channel)
{
    _lastToRadioId = -1;
    ...
    Serial.printf("bitrate: %d channel: %d\n", bitrate, channel);
...
void myNRFLite::prepForTx(uint8_t toRadioId, SendType sendType)
{
...
uint8_t myNRFLite::waitForTxToComplete()
{
...
uint8_t myNRFLite::readRegister(uint8_t regName)
{
    uint8_t data;
...
void myNRFLite::readRegister(uint8_t regName, void *data, uint8_t length)
{
    spiTransfer(READ_OPERATION, (R_REGISTER | (REGISTER_MASK & regName)), data, length);
}

void myNRFLite::writeRegister(uint8_t regName, uint8_t data)
{
    writeRegister(regName, &data, 1);
}

void myNRFLite::writeRegister(uint8_t regName, void *data, uint8_t length)
{
    spiTransfer(WRITE_OPERATION, (W_REGISTER | (REGISTER_MASK & regName)), data, length);
}

void myNRFLite::spiTransfer(SpiTransferType transferType, uint8_t regName, void *data, uint8_t length)
{
...
uint8_t myNRFLite::usiTransfer(uint8_t data)
{
    #if defined(__AVR_ATtiny45__) || defined(__AVR_ATtiny85__) || defined(__AVR_ATtiny44__) || defined(__AVR_ATtiny84__)
...
uint8_t myNRFLite::twoPinTransfer(uint8_t data)
{
...
void myNRFLite::printRegister(const char name[], uint8_t reg)
{
    String msg = name;
    msg += " ";
